...
class SprintKPI{
    ...
    public function getKPIData($pieData, $tech, $tw, $date) {
        return self::generateKPIData($pieData, $tech, $tw, $date);
    }
	...
	static function generateKPIData($pieData, $tech, $tw, $date){
		...
		if($tech == 'GSM'){
			...
			//		$kpiData[] = $val;
			//	}else{
                if(isset($p["switch_id"]) && trim($p["switch_id"]) != '') {
                    $db = substr($p["switch_id"], 0, 2);
                } elseif(isset($p["key"]) && trim($p["key"])) {
                   $db = substr($p["key"], 0, 2);
                }
            //var_dump($db);exit;
...
//var_dump($db);exit;
		...
		if($tech == 'CDMA')
		{
		  ...
		  $sql = 'Select DB, max(IP) as IP, string_agg(Cascad,\',\') as cascads from lookup where Cascad in ('.$cells.')
				  and DB != \'\' group by DB;';
		  $dbips = Doo::db()->fetchAll($sql);
		  foreach($dbips as $dbip)
		  {
		   $db = $dbip['db'];
		   $ip = $dbip['ip'];
		   $cascads = $dbip['cascads'];
		   $cascads = explode(",", $cascads);
		   $kpiRequest[] = array('key' => $db, 'post' => array('ip' => $ip, 'db' => $db, 'sql' => self::getcdmaKPISQL(join("','", array_keys($cascads)),  $tw,$date, $db)));
		  }
		}
...
//var_dump($sql); exit;
	...
	static function getcdmaKPISQL($key, $tw, $date, $db){
		$sql = "";
		//convert
		$mysql_date = trim(date('Y-m-d ', strtotime($date)));
		if($tw == 'Month'){
			$sql = "SELECT
					SiteCascadeCode as 'key',
					SectorId as sector,
					SUM(num_Dropped_Call_Voice) AS Drops,
					SUM(DropPerc__N) / SUM(DropPerc_) AS Drop_perc,
					(SUM(num_Blocked_Call_Voice) + SUM(Resource_Blocks)) AS Blocks,
					(SUM(Perc_Blocked_Call_Voice_N) + SUM(Perc_Resource_Blocks_N)) / (SUM(Perc_Blocked_Call_Voice) + SUM(Perc_Resource_Blocks)) AS Block_perc,
					SUM(Daily_MOU_Voice) AS MOU,
					SUM(Silent_Re_Orig_Attempts_Voice) AS ATT
				FROM
					$db.CDMAPerf_Voice_Sector_Daily
				WHERE
					SiteCascadeCode IN ( '$key' ) AND MONTH('$mysql_date') = MONTH(DayKey) AND YEAR('$mysql_date') AND YEAR(DayKey)
				GROUP BY SiteCascadeCode , SectorId";
		}
		...
		if($tw == 'Week'){
			$sql = "SELECT
				...
				FROM
					$db.chaddRFISS_1xPerf
				WHERE
					SiteCascadeCode IN ( '$key' ) AND WEEK('$mysql_date') = WEEK(Daykey) AND YEAR('$mysql_date') AND YEAR(Daykey)
				GROUP BY SiteCascadeCode , SectorId";
		...
		if($tw == 'Day'){
			$sql = "SELECT
				...
				FROM
					$db.chaddRFISS_1xPerf
				WHERE
					SiteCascadeCode IN ( '$key' ) AND Daykey = '$mysql_date'
				GROUP BY SiteCascadeCode , SectorId";
		}
//var_dump($sql); exit;
		return $sql;
	...
	static function getumtsKPISQL($key, $tw, $date, $db){
		...
		if($tw == 'Month'){
			$sql = "SELECT
					...
					SUM(Voice_Access_Failure_rate_N) / SUM(Voice_Access_Failure_rate_D) as Voice_AFR,
					SUM(HSDPA_Drop_Rate_new_N) / SUM(HSDPA_Drop_Rate_new_D) as HSDPA_PS_DCR,
					...
					SUM(PS_Interactive_Drop_Rate_N) / SUM(PS_Interactive_Drop_Rate_D) as PS_RAB_DR,
                                        SUM(PS_Interactive_Access_Failure_Rate_new_N) / SUM(PS_Interactive_Access_Failure_Rate_new_D) as PS_AFR
				FROM
					$db.umtsRNCperf
				WHERE
