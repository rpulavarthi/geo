...
//var_dump('data<pre>' . print_r($data, true) . '</pre>');
//var_dump('kpirequest<pre>' . print_r($kpiRequest, true) . '</pre>'); exit;
			...
			foreach($res as $db => $rr){
				foreach(json_decode($rr, true) as $r){
					...
					//save
					$data[$key][$sector][$carrier][$band] = $r;
				}
			}
...
//var_dump('<pre>' . print_r($data, true) . '</pre>'); exit;
		return $data;
		...
		if($tw == 'Month'){
			$sql = "SELECT
				...
				FROM
					$db.umtsRNCperf
				WHERE
					substr(trim(Site_Name),2, 8) IN ( '$key' ) AND MONTH('$mysql_date') = MONTH(Daykey) AND YEAR('$mysql_date') AND YEAR(Daykey)
				GROUP BY Site_Name , Sector_Id,Carrier_Id,band";
		}
			...
			$sql = "SELECT
					...
					Carrier_Id as carrier,
                                ...
                                FROM
                                        $db.umtsRNCperf
                                WHERE
                                        substr(trim(trim(Site_Name),2, 8) IN ( '$key' ) AND week('$mysql_date') = week(Daykey) AND YEAR('$mysql_date') AND YEAR(Daykey)
                                GROUP BY Site_Name , Sector_Id,Carrier_Id,band";
			...
			$sql = "SELECT
					...
					Carrier_Id as carrier,
                                ...
                                FROM
                                        $db.umtsRNCperf
                                WHERE
                                        substr(trim(Site_Name),2, 8) IN ( '$key' ) AND Daykey = '$mysql_date'
                                GROUP BY Site_Name , Sector_Id, Carrier_Id,band";
		}
//var_dump($sql); exit;
		return $sql;
